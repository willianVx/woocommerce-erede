...
//		include_once dirname( __FILE__ ) . '/../model/class-wc-erede-config-static.php';
	...
	*/
	private function getInstallmentsHtml( $orderTotal ) {
		...
		for ( $i = 1; $i <= $installments; $i++ ) {
			$installmentValue = $this->getInstallmentValue($this->get_parcelas_com_juros($orderTotal, $i));
			
			if($installmentValue >= $minimumInstallmentValue) {
				$optionsCount++;
				
				$html .= '<option value="' . $i . '">' . 
							sprintf( __( '%sx of %s ', 'messages' ), $i, sanitize_text_field( wc_price( $installmentValue ) )) . 
						'</option>';
			}
		}
		...
		{
			$html .= '<option value="1">' . 
						sprintf( __( '%sx of %s ', 'messages' ), 1, sanitize_text_field( wc_price( $orderTotal ) )) . 
					'</option>';
		}
	...
	private function getInstallmentValue($value) {
 	...
 	public function processPayment($order_id, $orderTotal) {
 		try{
			...
			$valid &= $this->validateCreditCpf(preg_replace('/[^0-9]/', '', $erede_credit_cpf));
	...
	*/
	...
	private function updateCustomFields( $order_id, $request, $response, $cpf) {
		...
		if ( ! empty( $response->getTid() ) ) {
				
			update_post_meta( $order_id, WC_Erede_Post_Meta_Field::Tid, $response->getTid() );
                        ###DEPRECATED ereg_replace replaced by preg_replace###
